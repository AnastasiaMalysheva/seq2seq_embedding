...
class Seq2SeqLSTM(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, batch_size=64, epochs=100, latent_dim=300, validation_split=0.2, grad_clipping=100.0, lr=0.001,
    ...
    def fit(self, X, y, **kwargs):
        ...
        #nput_characters = sorted(list(input_characters | input_characters_))
        ...
        if self.verbose:
         ...
         #   print(u'Number of unique input tokens:', len(input_characters))
            ...
            print(u'')
        tokenizer = Tokenizer(nb_words=kwargs['max_n_words'])
        tokenizer.fit_on_texts(X)
        sequences = tokenizer.texts_to_sequences(X)
        word_index = tokenizer.word_index
        data = pad_sequences(sequences, maxlen=kwargs['max_n_words'])
        indices = np.arange(data.shape[0])
        np.random.shuffle(indices)
        data = data[indices]
        nb_validation_samples = int(self.validation_split * data.shape[0])
        X = data[:-nb_validation_samples]
        X_eval_set = data[-nb_validation_samples:]
        if 'pretrained_file' in kwargs:
            model = FastText.load(kwargs['pretrained_file'])
        ...
        encoder_inputs = Input(shape=(kwargs['max_n_words'],))
        embedding_layer = Embedding(len(word_index) + 1, 300, weights=[embedding_matrix], input_length=kwargs['max_n_words'], trainable=False)
        emb_input = embedding_layer(encoder_inputs)
        encoder = LSTM(self.latent_dim, return_sequences=False, return_state=True)
        encoder_outputs, state_h, state_c = encoder(emb_input)
        encoder_states = [state_h, state_c]
        decoder_inputs = Input(shape=(kwargs['max_n_words'],))
        embedding_layer = Embedding(len(word_index) + 1, 300, weights=[embedding_matrix],
                                    input_length=kwargs['max_n_words'], trainable=False)
        emb_input_dec = embedding_layer(decoder_inputs)
        decoder_lstm = LSTM(self.latent_dim, return_sequences=True, return_state=True)
        decoder_outputs, _, _ = decoder_lstm(emb_input_dec, initial_state=encoder_states)
